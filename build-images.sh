...
reponame="infisical"
APP_VERSION="v0.75.0-postgres"

# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-infisical container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-infisical; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-infisical -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/redis:7  docker.io/infisical/infisical:${APP_VERSION}" \
    "${container}"
# Commit the image
